...
float get_coordinates_xyz_from_char(int hand, int str_number, int motion_number, int xyz_number) {//文字から座標を取得
	...
	if(i<=71){//入力された文字が見つかった場合
		...
		printf("ひらがなで正しく入力してね：%c\n",inputString[str_number]);
		if(hand ==0)
			current_coord.coord[xyz_number] = flag_locate[7][hand][xyz_number];
		else
			current_coord.coord[xyz_number] = flag_locate[1][hand][xyz_number];
